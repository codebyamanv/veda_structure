        ...
        const fetchData = async () => {
            try {
                ...
                if (res.success) {
                    ...
                    setProductFeatures(
                        Array.isArray(data.productFeatures) ? data.productFeatures[0] : '',
                    )
                    setProductBenefits(
                        Array.isArray(data.productBenefits) ? data.productBenefits[0] : '',
                    )
                    setProductFaqs(Array.isArray(data.productFaqs) ? data.productFaqs[0] : '')
                    setProductShipping(
                        Array.isArray(data.productShipping) ? data.productShipping[0] : '',
                    )

                    setEnergization(
                        Array.isArray(data.energization) && data.energization.length > 0
                            ? data.energization
                            : [{ title: '', price: '', isHaveForm: false }],
                    )
        ...
        <form className="flex flex-col gap-6" onSubmit={handleSubmit}>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <div className="grid gap-3">
                    ...
                    />
                    {newImages.length > 0 && (
                        <p className="text-sm text-gray-600">{newImages.length} files selected</p>
                    )}
                </div>
            </div>
            ...
            <div className="grid gap-3">
                <div className="flex justify-between items-center">
                    ...
                    <Button
                        type="button"
                        variant="ghost"
                        className="capitalize border hover:text-white hover:bg-orange-500"
            ...
            <Button
                type="submit"
                className="px-6 py-2 bg-orange-600 text-white font-bold w-fit ml-auto rounded mt-4"
            >
                Update Product
            </Button>
        </form>
