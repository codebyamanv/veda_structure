"use client"
import { deleteBracelet } from "@/apis/controllers/braceletController.js"
    ...
    return (
        <div>
            <div className="flex justify-end">
                <Link href="/admin/add-bracelet" className="my-2 inline-block rounded bg-orange-500 px-4 py-2 font-bold text-white transition-all hover:scale-105">
                    Add Bracelet
                </Link>
            </div>
                ...
                {bracelet?.map((item) => (
                    <div className="relative rounded-2xl bg-white shadow-lg" key={item?._id}>
                        <div className="m-2 flex justify-end gap-2">
                            <Link
                                href={`/admin/edit-bracelet/${item._id}`}
